  ...
  return (
    <div className="min-h-screen pt-32 pb-20 px-6 bg-background">
      <div className="max-w-7xl mx-auto">
        <header className="mb-12 flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            <h1 className="text-4xl md:text-5xl font-black font-outfit tracking-tighter italic uppercase text-white">
              {t.catalog.title}{" "}
              <span className="text-cedra-500">{t.catalog.titleAccent}</span>
            </h1>
        ...
        {/* Trending Section */}
        {trendingProducts.length > 0 && !searchQuery && (
          <div className="mb-12">
            <div className="flex items-center gap-2 mb-6 text-cedra-500 font-bold uppercase tracking-widest text-xs">
              <TrendingUp size={16} />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {trendingProducts.slice(0, 4).map((product, index) => (
                ...
                >
                  <ProductCard
                    id={product.id}
                    name={typeof product.name === 'string' ? product.name : (product.name as any)?.en || 'Product'}
                    price={product.price}
                    category={product.category_id || "Trending"}
                    sku={product.sku}
                  />
                </motion.div>
              ))}
        ...
        >
          ...
          <button
            type="submit"
            className="px-8 bg-cedra-500 text-white rounded-2xl font-black uppercase tracking-widest text-xs hover:bg-cedra-600 transition-colors"
          >
            {t.hero.searchButton}
          </button>
        </form>
        ...
        <div className="mb-8 flex items-center justify-between border-b border-white/5 pb-4">
          <div className="text-xs text-white/40 font-bold uppercase tracking-widest">
            {isLoading
              ? t.catalog.syncing
              : `${t.bestSellers.popular} ${products.length} ${t.catalog.found}`}
          </div>
        ...
        {isLoading ? (
          ...
          <div className="py-32 text-center bg-white/[0.02] rounded-[3rem] border border-dashed border-white/10">
            ...
            <h3 className="text-2xl font-bold mb-2 text-white">
              {t.catalog.noInventory}
            </h3>
            <p className="text-white/40 max-w-sm mx-auto text-sm">
              {t.catalog.trySearch} "{searchQuery}".
            </p>
          </div>
        )}
      </div>
