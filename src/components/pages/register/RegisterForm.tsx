  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.requires_2fa) {
        ...
        console.log(
          "No token in response, might need further action:",
          response,
        );
        setError(
          "Registration successful, but no session token received. Please try logging in.",
        );
      }
    } catch (err: any) {
    ...
    <div className="bg-white/[0.02] border border-white/5 p-10 rounded-[2.5rem] shadow-2xl backdrop-blur-3xl">
      ...
      <div className="mt-8 pt-8 border-t border-white/5 text-center">
        <p className="text-white/40 text-[10px] font-bold uppercase tracking-widest">
          ...
          <Link
            href="/login"
            className="text-cedra-500 ml-2 hover:underline"
          >
            {t.auth.signIn}
          </Link>
        </p>
